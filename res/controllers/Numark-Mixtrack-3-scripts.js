...
// =====================================================================
// === CUSTOM MOD: summary of local modifications (autonomously generated by an AI agent at the device owner's request) ===
//
// 1. FORCE 4 DECKS AT INIT   : init() sets [Master] num_decks = 4 so both hidden
//                              layers ([Channel3]/[Channel4]) always exist.
// 2. WHEEL BUTTON DUAL ACTION: SHORT press  = jog scratch-mode toggle (original behavior).
//                              HOLD >= 500ms = DDJ-800-style deck layer swap:
//                              left physical side swaps Deck 1 <-> 3,
//                              right physical side swaps Deck 2 <-> 4.
//                              Legacy SHIFT+TAP layer swap (bpmTap) still works.
// 3. LAYER-FIX               : PADLoopButton, toggleJogMode (slip) and PFLButton
//                              (quantize) now resolve the virtual deck via deckFromGroup
//                              and act on deck.group, so they keep working after a layer swap.
// 4. COLOR FX / FILTER KNOB (v2-cfx): unshifted filter knob drives ONLY the per-deck quick
//                              effect super knob ([QuickEffectRack1_[ChannelN]] super1).
//                              Stock default = Filter loaded => stock Rekordbox filter behavior;
//                              load any other CFX via the skin's CFX selector => Rekordbox
//                              USER-mode color FX on that deck. (The old Effect Unit 3 blend
//                              hack is fully removed.)
// 5. PAD FX (UNIT 4, v2-padfx): PAD MODE (hold) + hotcue pads = Rekordbox-style 3-slot stack:
//                              pads 1-3 HOLD = momentary gates of Unit 4 slots 1-3 (LED while
//                              held), pad 4 TAP = toggle [Controls],PadFxEditMode (ON = skin
//                              knobs can shape Unit-4 depths, OFF = frozen; LED mirrors state).
...
// (End of AI-generated modification notes.)
// =====================================================================
...
// === CUSTOM MOD (CDJ-layer): hold WHEEL button this long to swap deck layer (1<->3 / 2<->4) ===
// WHEEL button behavior: SHORT press = scratch-mode toggle, HOLD >=Ns = DDJ-800-style
// deck layer swap (left 1<->3, right 2<->4), SHIFT+TAP legacy swap also works (see bpmTap).
var wheelLayerHoldMs = 500; // ms, same convention as LongShortBtn's default longPressThreshold
...
var NumarkMixtrack3 = {
    group: "[Master]",
    // === CUSTOM MOD (v2-padfx): set true once Effect Unit 4 has been armed by a Pad FX button ===
...
NumarkMixtrack3.init = function(id, debug) {
    ...
    // === CUSTOM MOD (CDJ-layer): force 4 decks so the hidden layers ([Channel3]/[Channel4]) always exist ===
...
// === CUSTOM MOD (CDJ-layer): SHORT press keeps the original jog-mode toggle,
// === CUSTOM MOD (CDJ-layer): holding >= wheelLayerHoldMs swaps the deck layer (1<->3 / 2<->4)
NumarkMixtrack3.toggleJogMode = function(channel, control, value, status, group) {
    var deck = NumarkMixtrack3.deckFromGroup(group);

    if (value === DOWN) {
        // === CUSTOM MOD (CDJ-layer): only remember press time on DOWN; act on release ===
        NumarkMixtrack3.wheelDownTimes[group] = Date.now();
        return;
    }

    // value is UP here: measure how long the button was held
    // === CUSTOM MOD (audit-fix): timestamps keyed by the physical side's static group,
    // so a SHIFT+TAP swap between DOWN and UP cannot corrupt hold measurement
    var wheelDownTime = NumarkMixtrack3.wheelDownTimes[group];
    var held = (typeof wheelDownTime === "number") ? (Date.now() - wheelDownTime) : -1;

    if (held >= wheelLayerHoldMs) {
        // === CUSTOM MOD (CDJ-layer): DDJ-800-style layer swap using the incoming static group,
        // same pattern as SHIFT+TAP in bpmTap (left side 1<->3, right side 2<->4)
        // === CUSTOM MOD (audit-fix): end any live scratch session on the OLD deck first,
        // so the later platter-release cannot ramp out scratch mode on the wrong deck
        if (deck.touch) {
            engine.scratchDisable(deck.decknum, true);
            deck.touch = false;
            deck.seekingfast = false;
        }
        var newDeckNum = (deck.decknum + 1) % 4 + 1;
        NumarkMixtrack3.deckGroup[group] = "[Channel" + newDeckNum + "]";
        NumarkMixtrack3.initDeck(NumarkMixtrack3.deckGroup[group], true);
        engine.setValue("[Master]", "num_decks", 4);
        print("CUSTOM MOD: physical side swapped to [Channel" + newDeckNum + "]");

        // resync the WHEEL LED to the NEW virtual deck
        var newDeck = NumarkMixtrack3.deckFromGroup(group);
        newDeck.LEDs.jogWheelsInScratchMode.onOff(newDeck.jogWheelsInScratchMode ? ON : OFF);
        return;
    }

    if (deck.shiftKey) {
        // === CUSTOM MOD (layer-fix): toggle slip on the resolved virtual deck (deck.group),
        // not the raw incoming group, so SHIFT+WHEEL still works after a layer swap
        script.toggleControl(deck.group, "slip_enabled");
    } else {
...
// === CUSTOM MOD (v2-padfx): Rekordbox-style 3-slot instant Pad FX stack on the C++-locked Effect Unit 4 ===
// Unit 4 ignores continuous parameter updates from knobs/UI, but enabled/routing toggles still work,
...
NumarkMixtrack3.padFxPad = function(deck, pad, value) {
    ...
    var slotGroups = {
        1: "[EffectRack1_EffectUnit4_Effect1]",
        2: "[EffectRack1_EffectUnit4_Effect2]",
    ...
    if (pad >= 1 && pad <= 3) {
        ...
        var editMode = engine.getValue("[Controls]", "PadFxEditMode");
        engine.setValue("[Controls]", "PadFxEditMode", editMode ? 0 : 1);
        print("PadFX: edit mode " + (editMode ? "OFF - Unit 4 depths frozen" : "ON - Unit 4 depths editable"));
        deck.LEDs.hotCue4.onOff(editMode ? OFF : ON);
    }
};
...
NumarkMixtrack3.PADLoopButton = function(channel, control, value, status, group) {
    ...
    if (value === DOWN && deck.trackLoaded()) {
        ...
        // === CUSTOM MOD (layer-fix): read loop state from the resolved virtual deck (deck.group),
        // not the raw incoming group, so pads keep working after a layer swap
        if (engine.getValue(deck.group, "beatloop_" + loopsizeNew + "_enabled")) {
...
NumarkMixtrack3.InstantFXOff = function(channel, control, value, status, group) {
    ...
    // === CUSTOM MOD (v2-padfx): also kill any held Unit 4 pad-FX washes and reset their pad LEDs ===
    for (var u = 1; u <= 4; u++) {
        ...
        if (padDeck && padDeck.LEDs) {
            ...
            padDeck.LEDs["hotCue3"].onOff(OFF);
        }
    }
    // === CUSTOM MOD (v2-padfx): exit Pad-FX edit mode so the skin's Unit-4 depth knobs freeze again ===
...
NumarkMixtrack3.PFLButton = function(channel, control, value, status, group) {
    ...
    if (value === DOWN) {
        if (deck.shiftKey) {
            // === CUSTOM MOD (layer-fix): toggle quantize on the resolved virtual deck (deck.group),
            // not the raw incoming group, so SHIFT+PFL keeps working after a layer swap
            script.toggleControl(deck.group, "quantize");
        } else {
